import React from "react";
import award01 from "../assets/award01.png";
import award02 from "../assets/award02.png";
import award03 from "../assets/award03.png";
import titleLeft from "../assets/tittle_left.png";
import titleRight from "../assets/tittle_right.png";

const awards = [
  {
    awardImg: award01,
    title: "Best Forex Trader in Sri Lanka",
    description: "Honoring exceptional performance, innovation, and market impact."
  },
  {
    awardImg: award02,
    title: "Best Crypto Trader",
    description: "Recognizing groundbreaking strategies and outstanding crypto performance."
  },
  {
    awardImg: award03,
    title: "Top Female Trader",
    description: "Celebrating leading female talents showing skill, growth, and discipline."
  },
  {
    awardImg: award03,
    title: "Top Young Trader",
    description: "Highlighting young achievers with remarkable trading skills and potential."
  },
  {
    awardImg: award03,
    title: "Consistent Performer",
    description: "Acknowledging traders who deliver steady and reliable results."
  },
  {
    awardImg: award03,
    title: "Best Risk Manager",
    description: "Honoring mastery in minimizing losses and managing trading risks."
  },
  {
    awardImg: award03,
    title: "Best Trade of the Contest",
    description: "Recognizing the most impressive and strategic trade executed."
  },
  {
    awardImg: award03,
    title: "Best Scalper",
    description: "Celebrating fast, precise, and high-frequency trading excellence."
  },
  {
    awardImg: award03,
    title: "Best Swing Trader",
    description: "Awarding exceptional medium-term trading strategy and execution."
  },
  {
    awardImg: award03,
    title: "Community Favorite Award",
    description: "Honoring the trader most admired and loved by the community."
  }
];


export default function AwardCategory1() {
  return (
    <div className="flex flex-wrap gap-6 justify-start px-4 mt-10">
      {awards.map((award, idx) => (
        <div
          key={idx}
          className="w-[355px] h-[241px] shadow-2xl bg-[#535353]/20 outline outline-white text-white rounded-md p-4 flex flex-row"
        >
          {/* Left side */}
          <div className="shrink-0 flex items-center justify-center w-1/3">
            <img src={award.awardImg} alt="Award" className="w-20 h-auto" />
          </div>

          {/* Right side */}
          <div className="flex flex-col justify-center flex-1">
            <div className="flex items-center mb-3">
              <img src={titleLeft} alt="Left decor" className="w-6 h-auto" />
              <h2 className="font-alatsi text-lg text-white text-center flex-1">
                {award.title}
              </h2>
              <img src={titleRight} alt="Right decor" className="w-6 h-auto" />
            </div>
            <p className="font-alata text-sm text-gray-300">{award.description}</p>
          </div>
        </div>
      ))}
    </div>
  );
}
